...
<div id="sodr_pop" class="new_win">
	...
	<section id="anc_sodr_list">
		...
		<script language="Javascript">
			...
			function result_submit(result_cd,result_msg,enc_data) {
				...
				if( result_cd != '0000' ){
                    ...
                    uf_form_add(frmorderform, 'act_button', document.pressed); // 폼전송한 버튼 value가 필요하여 부득이하게 추가함.
					frmorderform.allat_enc_data.value = enc_data;
					frmorderform.action = tb_admin_url+"/pop_orderstatusupdate.php";
					frmorderform.method = "post";
					frmorderform.target = "_self";
					frmorderform.submit();
				}
			}
		</script>
		...
		<input type="hidden" name="allat_amt" value="<?php echo $amount['useprice']; ?>">
		...
		<?php
            ...
            echo PsysPay::getCancelFormHtml($od, $amount);
        } ?>
					...
					<tr>
						...
						<td>
							...
							switch($od['od_pg']) {
								...
								case 'allat':
									$pg_url  = 'https://www.allatpay.com/servlet/AllatBizV2/login/LoginCL';
									$pg_test = 'KG올앳 ';
									break;
...
function form_submit(f)
{
	...
	if(confirm("주문상태를 변경하시겠습니까?")) {
        var cancel_status = status == "주문취소" || status == "전체환불" || status == "전체반품";
		<?php if($od['od_tno'] && $od['od_pg'] == 'allat' && $od['paymethod'] == '신용카드') { // 올앳페이일때 ?>
		if( cancel_status ) {
			ftn_approval(f);
		} else {
			return true;
		...
		<?php
            ...
            echo PsysPay::getCancelFormJs($od,'admin');
		} else { ?>
		return true;
		<?php } ?>
	} else {
...
function fcancel_receive(json) {
    uf_form_add(frmorderform_f, 'act_button', document.pressed); // 폼전송한 버튼 value가 필요하여 부득이하게 추가함.
    <?=PsysPay::getCancelReceiveJs('frmorderform_f', 'json')?>
}

function uf_form_add(f, name, value) {
