...
if($_FILES['excelfile']['tmp_name']) {
    ...
    */
	...
	for($i=4; $i<=$data->sheets[0]['numRows']; $i++)
	{
        ...
        if(!$mb_id || !$gcode || !$gname || !$gcate || !$zone || !$isopen || !$buy_level || !$simg1) {
			$fail_line[] = $i."라인 필수값 없음";
            $fail_count++;
            continue;
        }
		...
		$goods_value_arr[] = $value;
		$goods_gcate[] = $gcate;
	}

	if(!$fail_count) {
		for($z=0; $z<count($goods_value_arr); $z++) {
			insert("shop_goods", $goods_value_arr[$z]);
			$gs_id = sql_insert_id();

			// 기존 카테고리 자료를 모두 삭제
			sql_query("delete from shop_goods_cate where gs_id = '$gs_id'", FALSE);

			$arr = explode(",", trim($goods_gcate[$z]));
...
<div class="tbl_frm02">
	...
	<?php if($fail_count > 0) { ?>
	<tr>
		<th scope="row">실패엑셀라인</th>
		<td><?php echo implode('<br>', $fail_line); ?></td>
	</tr>
	<tr>
