...
$value['sc_each_use']	= $_POST['sc_each_use']?$_POST['sc_each_use']:"0"; //묶음배송불가
...
if($option_count) {
    ...
    $sql = " insert into shop_goods_option
                    ( `io_id`, `io_type`, `gs_id`, `sell_mm`, `io_price`, `io_stock_qty`, `io_noti_qty`, `io_use` )
                VALUES ";
    for($i=0; $i<$option_count; $i++) {
        $sql .= $comma . " ( '{$_POST['opt_id'][$i]}', '0', '$gs_id', '{$_POST['sell_mm'][$i]}', '{$_POST['opt_price'][$i]}', '{$_POST['opt_stock_qty'][$i]}', '{$_POST['opt_noti_qty'][$i]}', '{$_POST['opt_use'][$i]}' )";
        $comma = ' , ';
    }
    sql_query($sql);
}
