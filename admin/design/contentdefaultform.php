...
<form name="frmcontentform" method="post" action="./design/contentformupdate.php" onsubmit="return frmcontentform_check(this);">
    ...
    <input type="hidden" name="pt_id" value="default">
    ...
    <div class="tbl_frm02">
        <table>
            ...
            <tbody>
            <tr>
                <th scope="row">페이지 이름(영어로)</th>
                <td><input type="text" name="page_id" value="<?php echo $co['page_id']; ?>" required itemname="페이지 이름" class="required frm_input" size="60"></td>
            </tr>
            <tr>
                <th scope="row">제목</th>
            ...
            <?php if($w == 'u') { ?>
                <tr>
                    ...
                    <td><input type="text" value="/bbs/content.php?co_id=<?=$co_id?>&page_id=<?=$co['page_id']?>&pt_id=<?=$co['pt_id']?>" readonly class="frm_input list2" size="60"> <a href="/bbs/content.php?co_id=<?=$co_id?>&page_id=<?=$co['page_id']?>&pt_id=<?=$co['pt_id']?>" target="_blank" class="btn_small grey">페이지 바로가기</a></td>
                </tr>
            <?php } ?>
            <tr>
