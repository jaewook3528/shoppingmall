...
if($od['od_pg'] != 'allat') return;

include_once(TB_SHOP_PATH.'/settle_allat.inc.php');

//Request Value Define
//----------------------
$at_amt = (int)get_session('tot_price'); //결제 금액을 다시 계산해서 만들어야 함(해킹방지), ( session, DB 사용 )
...
if( !strcmp($REPLYCD,$allat_res_cd) ){ // 성공
	...
	$tno			  = trim($SEQ_NO);
	$app_time		  = trim($APPROVAL_YMDHMS);
	$amount			  = trim($AMT);

	$logfile = fopen( TB_SHOP_PATH.'/allat/log/result_'.date("Ymd").".log", "a+" );

    fwrite( $logfile,"***************************************************************************\r\n");
	fwrite( $logfile,"[".date("Y/m/d H:i:s")."] 전체 결과값\r\n");
	fwrite( $logfile,"결과코드 : ".$REPLYCD."\r\n");
    fwrite( $logfile,"결과메세지 : ".iconv_utf8($REPLYMSG)."\r\n");
    fwrite( $logfile,"주문번호 : ".$ORDER_NO."\r\n");
    fwrite( $logfile,"승인금액 : ".$AMT."\r\n");
    fwrite( $logfile,"지불수단 : ".$PAY_TYPE."\r\n");
    fwrite( $logfile,"승인일시 : ".$APPROVAL_YMDHMS."\r\n");
    fwrite( $logfile,"거래일련번호 : ".$SEQ_NO."\r\n");
    fwrite( $logfile,"에스크로 적용 여부 : ".$ESCROW_YN."\r\n");
	fwrite( $logfile,"=============== [신용카드]\r\n");
	fwrite( $logfile,"승인번호 : ".$APPROVAL_NO."\r\n");
	fwrite( $logfile,"카드ID : ".$CARD_ID."\r\n");
	fwrite( $logfile,"카드명 : ".iconv_utf8($CARD_NM)."\r\n");
	fwrite( $logfile,"할부개월 : ".$SELL_MM."\r\n");
	fwrite( $logfile,"무이자여부 : ".$ZEROFEE_YN."\r\n");   //무이자(Y),일시불(N)
	fwrite( $logfile,"인증여부 : ".$CERT_YN."\r\n");      //인증(Y),미인증(N)
	fwrite( $logfile,"직가맹여부 : ".$CONTRACT_YN."\r\n");  //3자가맹점(Y),대표가맹점(N)
	fwrite( $logfile,"세이브 결제 금액 : ".$SAVE_AMT."\r\n");
	fwrite( $logfile,"포인트할인 결제 금액 : ".$CARD_POINTDC_AMT."\r\n");
	fwrite( $logfile,"=============== [계좌이체/가상계좌]\r\n");
	fwrite( $logfile,"은행ID : ".$BANK_ID."\r\n");
	fwrite( $logfile,"은행명 : ".iconv_utf8($BANK_NM)."\r\n");
	fwrite( $logfile,"현금영수증 일련 번호 : ".$CASH_BILL_NO."\r\n");
    fwrite( $logfile,"=============== [가상계좌]\r\n");
    fwrite( $logfile,"계좌번호 : ".$ACCOUNT_NO."\r\n");
    fwrite( $logfile,"입금계좌명 : ".iconv_utf8($INCOME_ACC_NM)."\r\n");
    fwrite( $logfile,"입금자명 : ".iconv_utf8($ACCOUNT_NM)."\r\n");
    fwrite( $logfile,"입금기한일 : ".$INCOME_LIMIT_YMD."\r\n");
    fwrite( $logfile,"입금예정일 : ".$INCOME_EXPECT_YMD."\r\n");
    fwrite( $logfile,"현금영수증신청 여부 : ".$CASH_YN."\r\n");
    fwrite( $logfile,"=============== [휴대폰결제]\r\n");
    fwrite( $logfile,"이동통신사구분 : ".$HP_ID."\r\n");
    //fwrite( $logfile,"=============== [상품권결제]\r\n");
    //fwrite( $logfile,"상품권 ID : ".$TICKET_ID."\r\n");
    //fwrite( $logfile,"상품권 이름 : ".iconv_utf8($TICKET_NAME)."\r\n");
    //fwrite( $logfile,"결제구분 : ".$TICKET_PAY_TYPE."\r\n");
	//fwrite( $logfile,"부분취소가능여부 : ".$PARTCANCEL_YN."\r\n");
	fclose( $logfile );
