...
EOF;
if($member['use_pg_long']) { // unltd
    $btn_frmline .= '<input type="submit" name="act_button" value="선택장기무이자수정" class="btn_lsmall bx-white" onclick="document.pressed=this.value">';
}
?>
...
<div class="tbl_head02">
	...
	<colgroup>
		...
		<col class="w90">
		<col class="w60">
    <?php if($member['use_pg_long']) { // unltd?>
        <col class="w50">
    <?php } ?>
	</colgroup>
	<thead>
    ...
    <?php if($member['use_pg_long']) { // unltd?>
        <th scope="col" rowspan="2">장기<br>무이자</th>
    <?php } ?>
	</tr>
	<tr class="rows">
    ...
    <?php if($member['use_pg_long']) { // unltd?>
        <td rowspan="2"><input type="checkbox" name="psys_long_use[<?php echo $i; ?>]" value="1" id="psys_long_use"<?php echo get_checked($row['psys_long_use'], "1"); ?>></td>
    <?php } ?>
	</tr>
	<tr class="<?php echo $bg; ?>">
