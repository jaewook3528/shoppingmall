	...
	<div class="sodr_print_pop_list">
		...
		<div class="tbl_head01">
			...
			*/
			$sql2 = " select c.*
					  from shop_cart as c
					  left join shop_order as o on (c.od_no = o.od_no)
		   			  where c.od_id = '{$row['od_id']}' and o.index_no is not null
		   			  group by c.gs_id
		              order by c.io_type, c.index_no ";
			$res2 = sql_query($sql2);
			while($row2=sql_fetch_array($res2)) {
				...
				if($row2['io_type'] == '1'){
					$ct_price = $row2['io_price'] ;
				}else{
					$ct_price =   $row2['io_price'] + $row2['ct_price'];
				}
			...
			<tr>
				...
				<td class="tar"><?php echo number_format($ct_price); ?></td>
