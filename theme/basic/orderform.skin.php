...
<div class="tbl_head02 tbl_wrap">
	...
	$goods_psys_long_no_use_cnt = 0; // 장기무이자가 아닌 제품 주문 카운드
	...
	for($i=0; $row=sql_fetch_array($result); $i++) {
        ...
        if( ! PsysPay::checkLongUse($gs) ) {
            $goods_psys_long_no_use_cnt++;
        }
        ...
        $it_name .= PsysPay::getLongUseStr($gs);
	...
	<div class="tbl_frm01 tbl_wrap">
		...
		<tr>
			...
			<td class="td_label">
                ...
                if($default['de_card_direct_use']) {
                    echo '<input type="radio" name="paymethod" id="paymethod_card_direct" value="신용카드(카드번호)" onclick="calculate_paymethod(this.value);"> <label for="paymethod_card_direct">신용카드(카드번호)</label>'.PHP_EOL;
                }
				if($default['de_hp_use']) {
					echo '<input type="radio" name="paymethod" id="paymethod_hp" value="휴대폰" onclick="alert(\'결제 연동작업중입니다.\'); return false; calculate_paymethod(this.value);"> <label for="paymethod_hp">휴대폰</label>'.PHP_EOL;
				}
				...
				PsysPay::printPaymethodForm('html', $goods_psys_long_no_use_cnt);
...
function fbuyform_submit(f) {
	...
	<?php PsysPay::printPaymethodForm('js'); ?>
