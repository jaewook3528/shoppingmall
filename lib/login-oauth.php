...
function get_login_oauth($type, $img='') {
	...
	if(!defined('APMS_SNS_LOGIN_OAUTH')) {
		...
		echo 'var opt = "width=" + ww + ",height=" + wh + ",left=0,top=0,scrollbars=0,toolbars=no,resizable=yes";'.PHP_EOL;
		...
		if($img == '1') { // Link
			switch($type) {
				...
				case 'kakao':
					...
					break;
                case 'psys':
                    $str = '<a href="javascript:'.$str.'" class="bt_psys"><span class="bt_ic"><img src="'.TB_IMG_URL.'/oauth_pop/ic_psys.png" alt="Sign in with '.$type.'"></span> 피시스 로그인</a>'.PHP_EOL;
                    break;
                case 'pay2pay':
                    $str = '<a href="javascript:'.$str.'" class="bt_kakao"><span class="bt_ic"><img src="'.TB_IMG_URL.'/ic_naver.png" alt="Sign in with '.$type.'"></span> 페이투페이 로그인</a>'.PHP_EOL;
                    break;
			}
		} else {
